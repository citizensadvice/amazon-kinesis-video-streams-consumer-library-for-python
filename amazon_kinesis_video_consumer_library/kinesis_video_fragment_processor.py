...
class KvsFragmentProcessor:
                    ...
                    for cluster_child in segment_child:
                        if cluster_child.name == "SimpleBlock":
                            simple_block_track_nr = (
                                self.get_audio_track_number_from_simple_block(cluster_child)
                            )
                            i += 1
    ...
    def save_connect_fragment_audio_track_from_customer_as_wav(
        self, fragment_dom, file_name_path_part
    ):
        """
        ...
        if track_number:
            ...
            self.save_connect_fragment_audio_track_as_wav(
                fragment_dom, track_number, file_name_path
            )

    def save_connect_fragment_audio_track_to_customer_as_wav(
        self, fragment_dom, file_name_path_part
    ):
        """
