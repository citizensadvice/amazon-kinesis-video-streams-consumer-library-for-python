...
class KvsPythonConsumerExample:
    ...
    def on_fragment_arrived(
        ...
        try:
            ...
            time_now = time.time()
            kvs_ms_behind_live = float(
                self.last_good_fragment_tags["AWS_KINESISVIDEO_MILLIS_BEHIND_NOW"]
            )
            producer_timestamp = float(
                self.last_good_fragment_tags["AWS_KINESISVIDEO_PRODUCER_TIMESTAMP"]
            )
            server_timestamp = float(
                self.last_good_fragment_tags["AWS_KINESISVIDEO_SERVER_TIMESTAMP"]
            )
            ...
            log.info("")
            log.info(
                f'####### Saving audio track "AUDIO_FROM_CUSTOMER" from Amazon Connect fragment as WAV to base path: {wav_file_base_path}'
            )
            self.kvs_fragment_processor.save_connect_fragment_audio_track_from_customer_as_wav(
                fragment_dom, wav_file_base_path
            )
            log.info(
                f'####### Saving audio track "AUDIO_TO_CUSTOMER" from Amazon Connect fragment as WAV to base path: {wav_file_base_path}'
            )
